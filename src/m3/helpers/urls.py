        ...
        if for_actions and hasattr(pack, 'actions'):
            for action in pack.actions:
                ...
                if isinstance(action, actions.Action):
                    ...
                    #TODO: здесь url может быть не правильный,
                    # т.к. мы сами создали экземпляр и у него
                    # нет ни Pack, ни Controller.
                    #TODO: поэтому берем его через absolute_url,
                    # т.к. он ищет экземпляр экшена во всех контроллерах
