...
#    print 'read_model(): grouped=%s, offset=%s, level_index=%s, level_keys=%s, begin=%s, end=%s' % (grouped, offset, level_index, level_keys, begin, end)
    ...
    if grouped:
        ...
        if filter:
            if field:
                if aggr:
                    query = data_provider.get_data().filter(filter).values(field).annotate(*aggr).annotate(count=Count("id"))
                else:
                    query = data_provider.get_data().filter(filter).values(field).annotate(count=Count("id"))
            else:
...
class GroupingRecordSQLAlchemyProvider(GroupingRecordProvider):
    ...
    '''
    def reader(self, *args, **kwargs):
        return read_sqlalchemy(*args, **kwargs)

    def counter(self, *args, **kwargs):
        return count_sqlalchemy(*args, **kwargs)

    def indexer(self, *args, **kwargs):
        return index_sqlalchemy(*args, **kwargs)
    
def index_sqlalchemy(grouped, level_index, level_keys, expandedItems, data_provider, sorting):
    '''
    построение индексов элементов в раскрытом уровне, только для группировок и для тех, которые раскрыты
    '''
    from sqlalchemy import and_
    query = data_provider.get_data().subquery()
    res = []
    if grouped and len(expandedItems) > 0:
        # для всех группировочных элементов будут использоваться ключи
        # если берется уровень больший, чем количество группировок, то выдаем просто записи
        ...
        if filter:
            new_query = data_provider.get_data().session.query(query.c[field]).select_from(query).filter(and_(*filter)).distinct()
        else:
            new_query = data_provider.get_data().session.query(query.c[field]).select_from(query).distinct()
        #сортировка 
        sort_fields = []
        if field and not field in sorting:
            #нет заданной сортировки, отсортируем по этому полю
...
def count_sqlalchemy(grouped, level_index, level_keys, expandedItems, data_provider):
    '''
    подсчет количества строк в раскрытом уровне
    '''
    from sqlalchemy import and_
...
#    print 'count_model(): grouped=%s, level_index=%s, level_keys=%s, expandedItems=%s' % (grouped, level_index, level_keys, expandedItems)
    ...
    if grouped:
        ...
        if level_index < len(grouped_ranges):
            ...
            # т.к. count(distinct не считает поля с null, то будем считать ВЛОБ!
            if filter:
                total_of_level = data_provider.get_data().session.query(query.c[field]).select_from(query).filter(and_(*filter)).distinct().count()
            else:
                total_of_level = data_provider.get_data().session.query(query.c[field]).select_from(query).distinct().count()
        else:
            if filter:
                total_of_level = data_provider.get_data().session.query(query).filter(and_(*filter)).count()
            else:
                total_of_level = data_provider.get_data().session.query(query).count()
...
def read_sqlalchemy(grouped, offset, level_index, level_keys, begin, end, data_provider, aggregates, sorting):
    '''
    вывод элементов дерева группировок в зависимости от уровня, ключевых элементов и интервала в уровне
    '''
    # специальный режим, когда считается общий итог по всем записям - не важна сортировка и группировка
    ...
    if level_index == -1:
        ...
        i = data_provider.get_data().session.query(*aggr).select_from(query)[0]
        item = data_provider.create_record()
        item.is_leaf = False
        item.index = None
        item.id = None
...
#    print 'read_model(): grouped=%s, offset=%s, level_index=%s, level_keys=%s, begin=%s, end=%s' % (grouped, offset, level_index, level_keys, begin, end)
    ...
    if grouped:
        ...
        if aggr:
            if filter:
                new_query = data_provider.get_data().session.query(*aggr).select_from(query).filter(and_(*filter)).group_by(*group)
            else:
                new_query = data_provider.get_data().session.query(*aggr).select_from(query).group_by(*group)
        else:
            if filter:
                new_query = data_provider.get_data().session.query(query).filter(and_(*filter))
            else:
                new_query = data_provider.get_data().session.query(query)
