...
__all__ = ('Message', 'GroupedMessage', 'Index',)
...
class GroupedMessage(MessageBase):
    ...
    def unique_users(self):
        '''
        Give unique users of group messages
        TODO:Make batter, by one query
        '''
        res = set()
        messages = self.message_set.all()
        for message in messages:
            obj_request = message.request
            res.add(obj_request.META.get('REMOTE_ADDR'))
        return len(res)
