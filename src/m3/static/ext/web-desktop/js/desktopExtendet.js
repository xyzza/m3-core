...
/**
 ...
 Ext.ux.StartMenu = Ext.extend(Ext.menu.Menu, {
     ...
     show : function(el, pos, parentMenu){
         ...
         var hardCodedPos = 36;
         posArray[1] = posArray[1] < 0 ? hardCodedPos||el.getHeight() : posArray[1]
         this.showAt(posArray ,parentMenu, false);
        
         var tPanelWidth = 100;
         var rightPadding = 2;
         var box = this.menuBWrap.getBox();
         this.menuPanel.setWidth(box.width-rightPadding);
 ...
 */
         ...
         this.startBtn = new Ext.Button({
             ...
             template: new Ext.Template(
                 ...
                 '<td class="ux-startbutton-center"><em class="{5} unselectable="on">',
                     '<button class="x-btn-text {2}" type="{1}" style="height:48px; width:48px; margin-top:-1px">{0}</button>',
                 '</em></td>',
 ...
 Ext.ux.TaskBar.TaskButton = function(win, el){
     ...
     Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
         iconCls: win.iconCls,
         text: Ext.util.Format.ellipsis(win.title, 32),
         tooltip: win.title,
         renderTo: el,
         handler : function(){
             if (!win.masked){
         ...
         template: new Ext.Template(
             ...
             '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">',
                 '<button class="x-btn-text {2}" type="{1}" style="height:32px;">{0}</button>',
             '</em></td>',
