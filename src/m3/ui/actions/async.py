...
from m3.contrib.m3_mutex import (capture_mutex, release_mutex, request_mutex,
...
if not 'm3.contrib.m3_mutex' in settings.INSTALLED_APPS:
    raise ImportError(u'Для работы асинхронных (фоновых) операций необходимо наличие приложения m3.contrib.m3_mutex в INSTALLED_APPS проекта.')


class IBackgroundWorker(Thread):
    '''
    Абстрактный класс для исполнения кода в фоновом режиме. Соответсвующие методы должны быть определены разработчиком.
    Тк класс представляет собой наследник Thread, нужно понимать что фактически при использовании
    будет создан новый тред, и учитывать это при доступе к ресурсам приложения, кои могут быть модифицированы
    другими тредами. Методы stop, start и ping должны возвращать экземпляры  AsyncOperationResult.
    Вообще говоря, можно реализовать данный класс подругому, главное чтобы был имплементирован интерфейс
    ...
    def run(self):
        '''
        Метод вызывается после вызоыва метода start() в новом потоке, здесь фактически следует
        писать собственный код
        '''
        raise NotImplementedError('Method run() must be overrided in  child class')
    ...
    def stop(self):
        ...
        '''
        raise NotImplementedError('Method stop() must be overrided in  child class')
    ...
    def request(self):
        ...
        '''
        raise NotImplementedError('Method request() must be overrided in  child class')
    ...
    def result(self):
        ...
        '''
        raise NotImplementedError('Method result() must be overrided in  child class')
    ...
    def _mutex_owner(self):
        return MutexOwner(name=IBackgroundWorker.MUTEX_GROUP, session_id=self.boundary)

    def _result_mutex_id(self):
        return MutexID(group=IBackgroundWorker.RESULT_MUTEX_GROUP, id=self.boundary)

    def _result_mutex_owner(self):
        return MutexOwner(name=IBackgroundWorker.RESULT_MUTEX_GROUP, session_id=self.boundary)
    ...
    def check_state(self):
        '''
        Проверяет состояние глобальной блокировки операции. Возвращает кортеж из двух
        элементов (is_active, status_data), где is_active=True/False - показывает активность
        установленной блокировки, и status_data - произвольный объект (чаще строка), в котором
        находится описание состоания операции.
        '''
        mutex_state, mutex = request_mutex(self._mutex_id())
        return (mutex_state != MutexState.FREE, mutex.status_data if mutex is not None else '')
    ...
    def unlock_result(self):
        release_mutex(mutex_id=self._result_mutex_id(),owner=self._result_mutex_owner(),)
...
class AsyncAction(Action):
    '''
    Экшен обработки запросов с клиента. В данном варианте инстанс класса воркера
    общий для всех http запросов экземпляра приложения, поэтому если один пользователь начал
    ...
    реализовать для каждого пользователя свою операцию, или же с помощью мутексов глобальную
    блокировку операции на уровне приложения.
    При наследовании должен быть определен атрибут worker_cls - класс наследующий/имплементирующий IBackgroundWorker
    '''
    COMMAND_START = 'start'
    COMMAND_STOP = 'stop'
    ...
    def context_declaration(self):
        return [
            ...
            ACD(name='boundary', type=str, required=True, default='default-boundary'),]

    def _worker_instance(self, request, context, *args, **kwargs):
        '''
        Метод, возвращающий экземпляр воркера фоновой операции. Данный метод можно переопределять
        для случаев, когда в воркер необходимо передавать дополнительные параметры.
        '''
