...
Генератор отчетов в Excel (m3.core.excel_reporting)
===================================================
...
В любую ячейку можно вставить картинку, если в комментарии указать тег **#Картинка КлючПути**. Где параметр КлючПути - это ключ в JSON контексте, по которому должен находиться полный путь к картинке, например::

	#Картинка logo.png    ИЛИ    #Картинка МояКартинка

Поддерживаются форматы jpg и png. Если ключ не найден в контексте или вместо имени файла пустая строка, то генератор пропустит вставку. Если путь есть и указан неверно, либо формат картинки не соответствует поддерживаемым, то произойдет ошибка. Поддерживается вставка из вертикальной развертки, задающейся тегами # и ##.

Вставка происходит точно по левому верхнему углу ячейки, при этом масштаб и размеры картинки остаются неизменными. Задать смещение и произвольный масштаб на данный момент нельзя из-за багов в Apache POI.

Пример::
	
	"logo.png": "d:\\_DEV\\eclipse_workspace\\JavaReport\\media\\logo.png",
	"МояКартинка": "d:\\_DEV\\eclipse_workspace\\JavaReport\\media\\bug.jpg",
